...
const Sidebar: React.FC<SidebarProps> = ({ config, setConfig, onClearChat, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Content */}
      <div className={`fixed inset-y-0 left-0 w-72 bg-zinc-950 border-r border-zinc-800 transform transition-transform duration-300 ease-in-out z-30 flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
              <Zap className="text-white fill-current" size={18} />
            </div>
            <h1 className="text-xl font-bold text-zinc-100 tracking-tight">Gemini Omni</h1>
          </div>
          <button 
            onClick={() => setIsOpen(false)} 
          ...
          <div className="space-y-4">
            ...
            <div className={`flex items-start justify-between gap-4 ${config.model === 'gemini-2.5-flash-image' ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
              ...
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  config.useThinking ? 'translate-x-6' : 'translate-x-1'
                }`} />
              </button>
            </div>
            ...
            <div className={`flex items-start justify-between gap-4 ${config.model === 'gemini-2.5-flash-image' ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  config.useGrounding ? 'bg-blue-600' : 'bg-zinc-700'
                }`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                  config.useGrounding ? 'translate-x-6' : 'translate-x-1'
                }`} />
              </button>
            </div>
        ...
        <div className="p-4 border-t border-zinc-800">
           <button 
             onClick={onClearChat}
             className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-zinc-900 hover:bg-red-900/20 text-zinc-400 hover:text-red-400 border border-zinc-800 hover:border-red-900/50 rounded-lg transition-all text-sm font-medium"
           >
             <Trash2 size={16} />
             Clear Conversation
           </button>
