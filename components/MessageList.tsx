...
const MessageList: React.FC<MessageListProps> = ({ messages, isLoading, currentModel, onExampleClick }) => {
  ...
  if (messages.length === 0) {
    ...
    let examples = [
      { title: "Quick Summary", prompt: "Summarize the key benefits of exercise in 3 bullet points." },
      { title: "Email Draft", prompt: "Write a polite decline to a wedding invitation." },
      { title: "Creative Idea", prompt: "Give me 5 catchy names for a coffee shop." }
    ];

    if (isCoding) {
      examples = [
        { title: "Refactor Code", prompt: "Refactor this component to use React Hooks and optimize performance." },
        { title: "Debug Error", prompt: "Explain this error: 'useEffect has a missing dependency'." },
        { title: "Generate Tests", prompt: "Write Jest unit tests for a utility function that formats dates." }
      ];
    } else if (isPro) {
      examples = [
        { title: "Complex Reasoning", prompt: "Explain the relationship between quantum mechanics and consciousness, citing major theories." },
        { title: "Visual Analysis", prompt: "Upload an image of a code snippet and explain what it does." },
        { title: "Strategic Analysis", prompt: "Analyze the potential impact of AGI on the global job market over the next decade." }
      ];
    } else if (isImage) {
      examples = [
        { title: "Image Editing", prompt: "Upload a photo and say: 'Add a pair of sunglasses to the dog'." },
        { title: "Style Transfer", prompt: "Upload a sketch and say: 'Turn this into a realistic 3D render'." },
        { title: "Object Removal", prompt: "Upload a photo and say: 'Remove the person in the background'." }
      ];
    } else if (isLite) {
      examples = [
        { title: "Rapid Q&A", prompt: "What is the capital of France?" },
        { title: "Text Cleanup", prompt: "Fix the grammar in this sentence: 'I goes to store yesterday'." },
        { title: "Simple List", prompt: "List 5 fruits that are red." }
      ];
    }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 p-4 md:p-8 animate-in fade-in duration-500 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-2.5 w-full max-w-2xl px-2 mb-auto md:mb-0">
          {examples.map((ex, i) => (
            <button 
              key={i}
              onClick={() => onExampleClick(ex.prompt)}
              className="flex items-center gap-3 p-3 bg-zinc-900/40 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 rounded-xl text-left transition-all group active:scale-[0.98]"
            >
              <div className="bg-zinc-800 p-2 rounded-lg group-hover:bg-zinc-700 transition-colors">
                <Sparkles size={14} className="text-zinc-400 group-hover:text-blue-400" />
              </div>
              <div className="flex-1 min-w-0">
                 <div className="text-xs font-medium text-zinc-300 group-hover:text-blue-100 truncate">{ex.title}</div>
      ...
      <div className={`pb-4 md:pb-6 px-3 md:px-4 ${index === 0 ? 'pt-4' : ''}`}>
        ...
        >
          ...
          <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 ${
            msg.role === Role.USER ? 'bg-zinc-800' : 'bg-gradient-to-br from-blue-600 to-purple-600 shadow-lg shadow-purple-500/20'
          }`}>
          ...
          <div className={`flex flex-col gap-1.5 max-w-[85%] md:max-w-[80%] ${msg.role === Role.USER ? 'items-end' : 'items-start'}`}>
            ...
            {(msg.text || (msg.role === Role.MODEL && (!msg.images || msg.images.length === 0))) && (
              <div className={`px-4 py-2.5 md:px-5 md:py-3.5 rounded-2xl relative group min-w-[60px] shadow-sm ${
                msg.role === Role.USER 
